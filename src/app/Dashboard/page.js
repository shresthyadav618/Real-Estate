...
<table className="mt-6 basic w-full">
    ...
    {p && p.map((elm)=>{
       ...
       return (
        <tr key={elm._id+'dashboard'}><td className="font-bold text-xl">{elm.name}</td> <td className="flex gap-x-4"><Link href={`/Dashboard/edit/${elm._id}`}  className="p-4 btn flex items-center  gap-x-4 bl bg-blue-900 font-bold rounded-lg">Edit  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
